...
class MidiData:
    def __init__(self, config):
        ...
        if config.data_type == 'note':
            self.dataset = NoteDataset(config)
        elif config.data_type == 'event':
            self.dataset = EventDataset(config)
    ...
    @staticmethod
    def Event2Midi(preds, save_path, velocity_scale=0.8):
        event_seq = EventSeq.from_array(preds)
...
DEFAULT_VELOCITY_RANGE = range(21, 109)  # 1~127
...
class EventSeq:
    ...
    # https: // github.com / tensorflow / magenta / blob / master / magenta / music / performance_encoder_decoder.py
